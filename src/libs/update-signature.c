...
update_complete_callback(gpointer user_data)
{
  ...
  if (ctx->main_page && GTK_IS_WIDGET(ctx->main_page))
  {
    updating_page_show_final_result(UPDATE_SIGNATURE_PAGE (ctx->main_page), data->message, icon_name);
  }

...
typedef struct {
  ...
  GMutex threats_mutex; // Only protect "threat_paths" field
  GList *threat_paths; // List of threats found during scan

  ScanPage *page; // The scan page object
...
add_threat_path(ScanContext *ctx, const char *path)
{
  ...
  threat_page_prepend_threat (ctx->page, ADW_ACTION_ROW(action_row)); // Add the action row to the list view

  /* Set file properties and connect signal */
  DeleteFileData *delete_data = delete_file_data_new(threat_page_get_list(ctx->page), action_row);
...
clear_threat_paths(ScanContext *ctx)
{
  ...
  threat_page_clear_list(ctx->page); // Clear the list view
...
scan_ui_callback(gpointer user_data)
{
  ...
  if (ctx->page && GTK_IS_WIDGET(ctx->page))
  {
    scanning_page_set_current_status (ctx->page, status_text);
  }
...
scan_complete_callback(gpointer user_data)
{
  ...
  gboolean is_cancelled = FALSE;
  is_cancelled = get_cancel_scan(ctx); // Get the cancellation state for thread-safe access

  const char *detail_text = NULL;
  if (is_cancelled) // Check if the scan has been cancelled
  {
    detail_text = gettext("User cancelled the scan");
  }

  if (ctx->total_threats > 0) // If threats found, output the list view
  {
    show_threat_page (ctx->page);
  }

  const char *icon_name = is_success && !is_cancelled  ? "status-ok-symbolic" : "status-warning-symbolic";
  
  scanning_page_set_final_result (ctx->page, data->message, detail_text, icon_name);
...
start_scan(ScanPage *page, const char *path)
{
  ...
  cancel_signal_bind(ctx->page, G_CALLBACK(set_cancel_scan), ctx);
